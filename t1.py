...
class randNetwk(ig.Graph):  # random network inherits from Graph
    ...
    def greedy_subNetwk(self):
        ...
        print
        'optimal number of clusters for this dendrogram ' + str(self.greedyNetwk.optimal_count)
        print
        "the modularity score for community network" + str(self.greedyNetwkClst.modularity)

    def connected(self):
        print
        'connected: ' + str(self.network.is_connected())
    ...
    def begin_growing(self, N):
        ...
        while (n <= N):
            ...
            for i in range(0, n):
                if sumDegree == 0:
                    prob.append(1.0 / (
                                n + 1))  ## only happens if sumDegree=0, n=1 then 1/2 connect 1/2 not, n=2 then 1/3 connect to 1, 1/3 connect to 2, or 1/3 not
                else:
                    if (self.network.degree()[
                        i] == 0):  ## possible that one old node has no connection at all (due to being unlucky)
    ...
    def forest_fire(self, param):
        try:
            ...
            print
            'diameter' + str(self.network.get_diameter())
        except:
            print
    ...
    def giant_connected(self):
        ...
        print
        'nodes in giant connected component: ' + str(len(self.giantcc.degree()))
        self.ratioGC_trueGraph = len(self.giantcc.degree()) * 1.0 / len(self.network.degree())
        print
        'ratio of |Gcc| over |G| ' + str(self.ratioGC_trueGraph)

    def randomWalk(self, stop):
        if (self.network is None):
            print
            "must create the network"
    ...
    def manyRandomWalks(self, numerate, stop):
        ...
        self.randomWalkersNode = np.ones(
            (numerate, stop + 1))  # plus one due to stop=10, then there are 11 total (due to starting location)
...
rand = randNetwk()
rand.powerLawGraph(1000, 999,
                   3)  ## 1000 nodes, 999 edges. In this undirected graph, new node i, connects to existing j with p_{ij} propto to degree(j)/ sum_k degree(k).
